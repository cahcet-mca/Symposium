...
symposiumSettingsSchema.statics.getSettings = async function() {
  try {
    ...
    if (!settings) {
      ...
      console.log('✅ Symposium settings created with default values');
    }
    return settings;
  } catch (error) {
...
// Method to update symposium name
symposiumSettingsSchema.statics.updateName = async function(newName, updatedBy = 'admin') {
  try {
    let settings = await this.getSettings();
    
    settings.symposiumName = newName;
    settings.updatedBy = updatedBy;
    settings.updatedAt = new Date();
    
    await settings.save();
    
    console.log(`📝 Symposium name updated to: ${newName}`);
...
// Method to update UPI ID
symposiumSettingsSchema.statics.updateUpiId = async function(upiId, updatedBy = 'admin') {
  try {
    let settings = await this.getSettings();
