...
const verifyPayment = async (req, res) => {
  try {
    ...
    const transaction = await Transaction.create({
      ...
      user: req.user._id,
      event: event._id,
      status: 'pending' // Changed from 'verified' to 'pending'
    });
    ...
    // STEP 9: Create registration in MongoDB (set as pending)
    ...
    const registration = await Registration.create({
      ...
      paymentStatus: 'pending', // Changed from 'verified' to 'pending'
      registrationStatus: 'pending' // Changed from 'confirmed' to 'pending'
    });
    console.log('✅ Registration saved with PENDING status:', registration._id);

    // STEP 10: Create Event Register entry in MongoDB (set as pending)
    ...
    // STEP 11: DO NOT increment event registered count yet - wait for admin approval
    // Commented out - count will be incremented when admin accepts
    // const updateResult = await Event.updateOne(
    //   { _id: event._id },
    //   { $inc: { registeredCount: 1 } }
    // );

    // STEP 12: DO NOT add event to user's registered events yet - wait for admin approval
    // await User.findByIdAndUpdate(req.user._id, {
    //   $push: { registeredEvents: event._id }
    // });

    // STEP 13: Save to Google Sheets Event Register with pending status
