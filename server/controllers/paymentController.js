...
const verifyPayment = async (req, res) => {
  try {
    ...
    // STEP 4: Check if user has an existing registration (pending or accepted)
    // Allow re-registration only if previous was rejected
    ...
    if (existingRegistration) {
      // If registration exists and is not rejected, block new registration
      if (existingRegistration.paymentStatus !== 'rejected') {
        console.log(`❌ User already has ${existingRegistration.paymentStatus} registration for this event`);
        return res.status(400).json({
          success: false,
          message: `You already have a ${existingRegistration.paymentStatus} registration for this event. ${existingRegistration.paymentStatus === 'pending' ? 'Please wait for admin approval.' : ''}`
        });
      } else {
        // If previous registration was rejected, allow re-registration
        console.log('✅ Previous registration was rejected. Allowing re-registration...');
        
        // Optionally: Delete or archive the old rejected registration
        // You can choose to keep it for history or delete it
        // await existingRegistration.deleteOne();
        // console.log('🗑️ Removed old rejected registration');
      }
    }
