...
/**
 * @desc    Get user's registrations (ALL statuses - pending, verified, rejected)
...
const getMyRegistrations = async (req, res) => {
  try {
    ...
    // Find ALL registrations for the logged-in user (no status filter)
    // This returns pending, verified, and rejected registrations
    ...
    console.log(`✅ Found ${registrations.length} total registrations for user ${req.user.email}`);
    console.log(`   📊 Status Breakdown:`);
    console.log(`      ⏳ Pending: ${pending}`);
    console.log(`      ✅ Accepted: ${verified}`);
    console.log(`      ❌ Rejected: ${rejected}`);

    // Optional: Log details of each registration for debugging
    registrations.forEach((reg, index) => {
      console.log(`      ${index + 1}. ${reg.eventName} - Status: ${reg.paymentStatus} (${reg.registrationStatus})`);
    });
...
const checkTimeConflict = async (req, res) => {
  try {
    ...
    // Only check conflicts with ACCEPTED/VERIFIED registrations
    const userRegistrations = await Registration.find({ 
      user: req.user._id,
      paymentStatus: 'verified' // Only check against accepted registrations
    }).populate('event');
...
const cancelRegistration = async (req, res) => {
  try {
    ...
    // Update event registered count (decrease by 1) - only if registration was verified
...
const checkIfRegistered = async (req, res) => {
  try {
    ...
    if (registration) {
      if (registration.paymentStatus === 'verified') {
        message = 'Already registered and accepted';
        canRegister = false;
      } else if (registration.paymentStatus === 'pending') {
        message = 'Registration pending approval';
        canRegister = false;
      } else if (registration.paymentStatus === 'rejected') {
        message = 'Previous registration was rejected - you can register again';
        canRegister = true;
      }
    }
    ...
    res.json({
      ...
      status: registration ? registration.paymentStatus : null,
...
const getUpcomingRegistrations = async (req, res) => {
  try {
    ...
    const registrations = await Registration.find({ 
      user: req.user._id,
      paymentStatus: 'verified' // Only show verified/upcoming events
    })
      .populate('event')
...
const getPastRegistrations = async (req, res) => {
  try {
    const registrations = await Registration.find({ 
      user: req.user._id,
      paymentStatus: 'verified' // Only show verified/completed events
    })
      .populate('event')
      .sort('-createdAt');
...
const getRegistrationCount = async (req, res) => {
  try {
    ...
    const count = await Registration.countDocuments({
      event: eventId,
      paymentStatus: 'verified' // Only count verified registrations
    });
