...
const AdminDashboard = () => {
  ...
  const fetchSettings = useCallback(async () => {
    try {
      ...
      console.log('Fetching settings with token:', adminToken.substring(0, 10) + '...');
  ...
  // Toggle registrations open/closed - FIXED VERSION
  const toggleRegistrations = async () => {
    try {
      ...
      console.log('Using token:', adminToken.substring(0, 10) + '...');
        ...
        <div className="sidebar-footer">
          ...
          <div className="toggle-container">
            <button 
              ...
              className={`toggle-btn ${registrationsOpen === true ? 'on' : 'off'}`}
              disabled={toggling}
              title={registrationsOpen === true ? 'Click to close registrations' : 'Click to open registrations'}
            >
              <span className="toggle-icon">{registrationsOpen === true ? '🔓' : '🔒'}</span>
              <span className="toggle-text">
                {toggling ? 'Updating...' : (registrationsOpen === true ? 'Registrations: ON' : 'Registrations: OFF')}
              </span>
            </button>
          </div>
